    ...
    public static class LanguageManager
    {
        ...
        private static void InitializeBuiltInLanguages()
        {
            ...
            var enDict = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "Roles", "Roles" }, { "RolePermission", "Role Permission" }, { "Tax Management", "Tax Management" },
                { "TaxManagement", "Tax Management" }, { "ActivityLog", "Activity Log" }, { "Activity Log", "Activity Log" },
                { "Year Closing", "Year Closing" }, { "Financial Year Closing", "Year Closing" }, { "App Language", "App Language" },
                { "Language Settings", "Language Settings" }, { "Save", "Save" }, { "Clear", "Clear" }, { "Delete", "Delete" },
                { "Update", "Update" }, { "Cancel", "Cancel" }, { "Print", "Print" }, { "Close", "Close" }, { "Search", "Search" },
                { "Filter", "Filter" }, { "Refresh", "Refresh" }, { "Hold", "Hold" }, { "Last Bill", "Last Bill" },
                { "Export", "Export" }, { "Import", "Import" }, { "Bill No", "Bill No" }, { "Customer Name", "Customer Name" },
                { "Customer", "Customer" }, { "Item Name", "Item Name" }, { "Barcode", "Barcode" }, { "Unit", "Unit" },
                { "Qty", "Qty" }, { "Unit Price", "Unit Price" }, { "Total Amount", "Total Amount" }, { "Amount", "Amount" },
                { "Net Amount", "Net Amount" }, { "Cost", "Cost" }, { "Selling Price", "Selling Price" }, { "Billed By", "Billed By" },
                { "Date & Time", "Date & Time" }, { "Date", "Date" }, { "Payment Mode", "Payment Mode" }, { "Total Sales", "Total Sales" },
                { "Total Orders", "Total Orders" }, { "Average Order", "Average Order" }, { "Total Profit", "Total Profit" },
                { "Items Sold", "Items Sold" }, { "Hold Item", "Hold Item" }, { "Ready", "Ready" }, { "Success", "Success" },
                { "Error", "Error" }, { "Warning", "Warning" }, { "Information", "Information" }, { "Select Language", "Select Language" },
                { "Import Custom Language", "Import Custom Language" }, { "Export Template", "Export Template" },
            ...
            var msDict = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "Import Custom Language", "Import Bahasa Tersuai" }, { "Export Template", "Eksport Templat" },
        ...
        public static string GetString(string textKey, string fallback = null)
        {
            ...
            {
                if (langDict.TryGetValue(textKey, out var translation) && !string.IsNullOrWhiteSpace(translation))
                {
                    return translation;
                }
            }
            ...
            {
                if (enDict.TryGetValue(textKey, out var enTranslation) && !string.IsNullOrWhiteSpace(enTranslation))
                {
            ...
            foreach (Control control in controls)
            {
                ...
                else if (control is DataGridView dgv)
                {
                    foreach (DataGridViewColumn col in dgv.Columns)
                    {
                        ...
                        {
                            col.HeaderText = GetString(col.HeaderText, col.HeaderText);
                        }
                    }
                }
                else if (control is UltraGrid uGrid)
                {
                    foreach (UltraGridBand band in uGrid.DisplayLayout.Bands)
                    {
                        foreach (UltraGridColumn col in band.Columns)
                        {
                            ...
                            {
                                col.Header.Caption = GetString(col.Header.Caption, col.Header.Caption);
                            }
                        }
                    }
                }
        ...
        {
            if (manager == null || manager.Ribbon == null) return;
            ...
            try
            {
                foreach (RibbonTab tab in manager.Ribbon.Tabs)
                {
                    ...
                    {
                        tab.Caption = GetString(tab.Caption, tab.Caption);
                    }
                    ...
                    foreach (RibbonGroup group in tab.Groups)
                    {
                        ...
                        {
                            group.Caption = GetString(group.Caption, group.Caption);
                        }
                        ...
                        foreach (ToolBase tool in group.Tools)
                        {
                            ...
                            {
                                tool.SharedProps.Caption = GetString(tool.SharedProps.Caption, tool.SharedProps.Caption);
                            }
                        }
                    }
                }
