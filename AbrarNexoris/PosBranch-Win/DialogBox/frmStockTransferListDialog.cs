...
{
    public partial class frmStockTransferListDialog : Form
    {
        private StockTransferRepository repo = new StockTransferRepository();
        private List<StockTransferMaster> transferListData = new List<StockTransferMaster>();

        public delegate void SelectedTransferHandler(int id, int stkTrNo, decimal totalAmount);
        public event SelectedTransferHandler OnTransferSelected;
        ...
        public frmStockTransferListDialog()
        {
            ...
            this.Load += frmStockTransferListDialog_Load;
        ...
        private void frmStockTransferListDialog_Load(object sender, EventArgs e)
        {
            LoadTransferData();
        ...
        private void LoadTransferData()
        {
            try
            {
                var rawData = repo.GetStockTransfers(SessionContext.CompanyId);
                if (rawData != null)
                {
                    var list = rawData.Select(d => new StockTransferMaster
                    {
                        Id = (int)d.Id,
                        StkTrNo = (int)d.StkTrNo,
                        TransferDate = (DateTime)d.TransferDate,
                        TotalAmount = Convert.ToDecimal(d.TotalAmount)
                    });
                    transferListData = ApplySort(list);
                }
                else
                {
                    transferListData = new List<StockTransferMaster>();
                }
                ultraGrid1.DataSource = transferListData;
            ...
            {
                UpdateStatus("Error loading transfers data: " + ex.Message);
            }
        }
        ...
        private void LoadSelectedTransfer()
        {
            if (ultraGrid1.ActiveRow == null) return;
            try
            {
                int id = Convert.ToInt32(ultraGrid1.ActiveRow.Cells["Id"].Value);
                int stkTrNo = Convert.ToInt32(ultraGrid1.ActiveRow.Cells["StkTrNo"].Value);
                decimal totalAmount = Convert.ToDecimal(ultraGrid1.ActiveRow.Cells["TotalAmount"].Value);

                OnTransferSelected?.Invoke(id, stkTrNo, totalAmount);
                this.DialogResult = DialogResult.OK;
                this.Close();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error loading selected transfer: " + ex.Message);
            }
        }

        private void ultraGrid1_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            LoadSelectedTransfer();
        }
        ...
        private void textBoxsearch_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (ultraGrid1.Rows.Count == 1) LoadSelectedTransfer();
        ...
        private void UpdateRecordCountLabel()
        {
            ...
            int totalCount = transferListData?.Count ?? 0;
        ...
        private void SetupUltraPanelStyle()
        {
            ...
            StyleIconPanel(ultraPanel4); // New/Edit - hidden
        ...
        private void ultraGrid1_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            try
            {
                ...
                string[] columnsToShow = { "StkTrNo", "TransferDate", "TotalAmount" };
                for (int i = 0; i < columnsToShow.Length; i++)
                {
                    if (e.Layout.Bands[0].Columns.Exists(columnsToShow[i]))
                    {
                        ...
                        {
                            case "StkTrNo": col.Header.Caption = "Transfer No"; col.Width = 150; break;
                            case "TransferDate": col.Header.Caption = "Date"; col.Width = 200; col.Format = "dd-MM-yyyy"; break;
                            case "TotalAmount": col.Header.Caption = "Amount"; col.Width = 150; col.Format = "N2"; col.CellAppearance.TextHAlign = HAlign.Right; break;
                        }
                    }
                }
            }
        ...
        private void InitializeSearchFilterComboBox()
        {
            ...
            comboBox1.Items.AddRange(new object[] { "All", "Transfer No" });
        ...
        {
            if (transferListData == null) return;
            try
            {
                string searchText = textBoxsearch.Text.Trim().ToLower();
                IEnumerable<StockTransferMaster> filtered = transferListData;
                if (!string.IsNullOrEmpty(searchText))
                {
                    string field = comboBox1.SelectedItem?.ToString() ?? "All";
                    filtered = transferListData.Where(b =>
                    {
                        switch (field)
                        {
                            case "Transfer No": return b.StkTrNo.ToString().Contains(searchText);
                            default: return b.StkTrNo.ToString().Contains(searchText);
                        }
                    });
                }
                ultraGrid1.DataSource = ApplySort(filtered);
        ...
        private void InitializeColumnOrderComboBox()
        {
            ...
            comboBox2.Items.AddRange(new object[] { "Transfer No", "Date", "Amount" });
        ...
        private void ReorderColumns(string selectedColumn)
        {
            ...
            ultraGrid1.SuspendLayout();
            var columnMap = new Dictionary<string, string> { { "Transfer No", "StkTrNo" }, { "Date", "TransferDate" }, { "Amount", "TotalAmount" } };
            string key = columnMap.ContainsKey(selectedColumn) ? columnMap[selectedColumn] : "StkTrNo";
        ...
        private void UltraPictureBox4_Click(object sender, EventArgs e)
        {
            ...
            if (ultraGrid1.DataSource is List<StockTransferMaster> list)
            {
                ultraGrid1.DataSource = ApplySort(list);
            }
        }

        private List<StockTransferMaster> ApplySort(IEnumerable<StockTransferMaster> source)
        {
            if (source == null) return new List<StockTransferMaster>();
            return isOriginalOrder
                ? source.OrderBy(b => b.StkTrNo).ToList()
                : source.OrderByDescending(b => b.StkTrNo).ToList();
        }
        ...
        private void PopulateColumnChooserListBox()
        {
            ...
            string[] standardColumns = { "StkTrNo", "TransferDate", "TotalAmount" };
            var displayNames = new Dictionary<string, string>
            {
                { "StkTrNo", "Transfer No" }, { "TransferDate", "Date" },
                { "TotalAmount", "Amount" }
            };
        ...
        private void ConnectNavigationPanelEvents()
        {
            ...
            ultraPanel5.Click += (s, e) => LoadSelectedTransfer();
            ultraPictureBox1.Click += (s, e) => LoadSelectedTransfer();
            label5.Click += (s, e) => LoadSelectedTransfer();
