...
namespace PosBranch_Win.DialogBox
{
    public partial class frmSalesListDialog : Form
    {
        ...
        private List<GetHoldBill> ApplySort(IEnumerable<GetHoldBill> source)
        {
            if (source == null) return new List<GetHoldBill>();
            return isOriginalOrder
                ? source.OrderBy(b => b.BillNo).ToList()
                : source.OrderByDescending(b => b.BillNo).ToList();
        }
        #endregion
