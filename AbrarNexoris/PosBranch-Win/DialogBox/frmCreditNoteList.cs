...
{
    /// <summary>
    /// Dialog for listing and selecting Credit Notes with professional UI features.
    /// </summary>
    public partial class frmCreditNoteList : Form
    {
        private CreditNoteRepository creditNoteRepo = new CreditNoteRepository();
        ...
        // Event for when a Credit Note is selected
        public delegate void CreditNoteSelectedHandler(int voucherId);
        public event CreditNoteSelectedHandler OnCreditNoteSelected;
        ...
        public frmCreditNoteList()
        {
            ...
            // Form key handler
            this.KeyDown += frmCreditNoteList_KeyDown;
        ...
        private void LoadCreditNotes()
        {
            try
            {
                UpdateStatus("Loading credit notes...");
                fullDataTable = creditNoteRepo.GetAllCreditNotes(Convert.ToInt32(DataBase.BranchId), SessionContext.FinYearId, 0, 1000);
                ...
                UpdateStatus($"Found {fullDataTable.Rows.Count} Credit Notes.");
            }
            catch (Exception ex)
            {
        ...
        private void ultraGrid1_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            try
            {
                ...
                ShowColumn(e, "CUST_NAME", "Customer Name", 250);
        ...
        private void SelectCreditNote()
        {
            ...
            try
            {
                ...
                if (voucherId > 0)
                {
                    OnCreditNoteSelected?.Invoke(voucherId);
                    this.DialogResult = DialogResult.OK;
                    this.Close();
                }
                else
                {
                    MessageBox.Show("Could not identify the selected Credit Note ID.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error selecting credit note: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void FilterData()
        {
            try
            {
                ...
                if (searchText == "Search credit notes...") searchText = "";
                ...
                if (!string.IsNullOrEmpty(searchText))
                {
                    ...
                    {
                        filter = $"PH_DOCNO LIKE '%{escapedText}%' OR CUST_NAME LIKE '%{escapedText}%' OR PH_ACCTCODE LIKE '%{escapedText}%'";
                    }
        ...
        private void InitializeColumnOrderComboBox()
        {
            ...
            {
                // Sorting logic if needed, or reorder column visible position
            };
        }
        ...
        private void frmCreditNoteList_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            if (textBoxsearch.Text == "Search credit notes...")
            {
                textBoxsearch.Text = "";
        ...
        private void textBoxsearch_LostFocus(object sender, EventArgs e)
        {
            ...
            {
                textBoxsearch.Text = "Search credit notes...";
        ...
        private void PopulateColumnChooserListBox()
        {
            ...
            foreach (UltraGridColumn col in ultraGrid1.DisplayLayout.Bands[0].Columns)
            {
                if (col.Hidden && !col.Key.Contains("Id") && col.Key != "OriginalRowOrder")
                {
                    ...
                    columnChooserListBox.Items.Add(new ColumnItem(col.Key, caption));
                }
            }
        }
        ...
        private void UltraGrid1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Y < 40) // Header region
            {
                UIElement element = ultraGrid1.DisplayLayout.UIElement.ElementFromPoint(e.Location);
                while (element != null && !(element is HeaderUIElement)) element = element.Parent;

                if (element is HeaderUIElement header)
                {
        ...
        {
            if (e.Button == MouseButtons.Left && isDraggingColumn && columnToMove != null)
            {
                if (Math.Abs(e.Y - startPoint.Y) > 50)
                {
                    columnToMove.Hidden = true;
                    isDraggingColumn = false;
                    PopulateColumnChooserListBox();
                    if (!columnChooserForm.Visible) ShowColumnChooser();
                    UpdateStatus($"Column '{columnToMove.Header.Caption}' hidden.");
                    columnToMove = null;
                }
            }
        }
        ...
        private void ColumnChooserListBox_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                var item = columnChooserListBox.Items[index] as ColumnItem;
                if (item != null)
                {
        ...
        private void ColumnChooserListBox_DragDrop(object sender, DragEventArgs e)
        {
            ...
            {
                var col = (UltraGridColumn)e.Data.GetData(typeof(UltraGridColumn));
                col.Hidden = true;
                PopulateColumnChooserListBox();
            }
        }
        ...
        private void UltraGrid1_DragDrop(object sender, DragEventArgs e)
        {
            ...
            {
                var item = (ColumnItem)e.Data.GetData(typeof(ColumnItem));
                if (ultraGrid1.DisplayLayout.Bands[0].Columns.Exists(item.ColumnKey))
                {
                    ultraGrid1.DisplayLayout.Bands[0].Columns[item.ColumnKey].Hidden = false;
                    PopulateColumnChooserListBox();
                }
            }
        }
        ...
        {
            if (e.Index < 0) return;
            var item = columnChooserListBox.Items[e.Index] as ColumnItem;
            if (item == null) return;
            ...
            Color bgColor = (e.State & DrawItemState.Selected) == DrawItemState.Selected ? Color.FromArgb(0, 120, 215) : Color.FromArgb(33, 150, 243);
        ...
        private void ConnectNavigationPanelEvents()
        {
            ...
            ultraPanel5.Click += (s, e) => SelectCreditNote();
            ultraPictureBox1.Click += (s, e) => SelectCreditNote();
            label5.Click += (s, e) => SelectCreditNote();
        ...
        private void FrmCreditNoteList_SizeChanged(object sender, EventArgs e) => PositionColumnChooserAtBottomRight();
        private void FrmCreditNoteList_FormClosing(object sender, FormClosingEventArgs e)
        {
