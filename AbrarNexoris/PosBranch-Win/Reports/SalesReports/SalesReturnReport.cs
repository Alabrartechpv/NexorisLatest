...
{
    /// <summary>
    /// Professional Sales Return Report Form with Hierarchical Master-Detail UltraGrid
    /// </summary>
    public partial class SalesReturnReport : Form
    {
        #region Private Fields
        private SalesReturnReportRepository reportRepository;
        ...
        private void InitializeForm()
        {
            try
            {
                reportRepository = new SalesReturnReportRepository();
        ...
        {
            dsHierarchical = new DataSet("SalesReturnReport");

            // Create Master Table (Sales Return Records)
            ...
            masterTable.PrimaryKey = new[] { masterTable.Columns["SReturnNo"] };
        ...
        /// <summary>
        /// Configure columns for Master Band (Sales Return Records)
        /// </summary>
        private void ConfigureMasterBandColumns(UltraGridBand masterBand)
        {
            // SReturnNo column
            if (masterBand.Columns["SReturnNo"] != null)
            {
                masterBand.Columns["SReturnNo"].Header.Caption = "Return No";
                masterBand.Columns["SReturnNo"].Width = 100;
            ...
            // Format date columns
            if (masterBand.Columns["SReturnDate"] != null)
            {
                masterBand.Columns["SReturnDate"].Format = "dd/MM/yyyy";
                masterBand.Columns["SReturnDate"].Header.Caption = "Return Date";
                masterBand.Columns["SReturnDate"].Width = 100;
            }
        ...
        {
            // Hide DetailID (auto-increment) and SReturnNo (foreign key)
            if (detailBand.Columns["DetailID"] != null)
                detailBand.Columns["DetailID"].Hidden = true;
            if (detailBand.Columns["SReturnNo"] != null)
            ...
            if (detailBand.Columns["SalesPrice"] != null)
            {
                ...
                detailBand.Columns["SalesPrice"].CellAppearance.TextHAlign = Infragistics.Win.HAlign.Right;
            }
        ...
        /// <summary>
        /// Load all sales return data without filters
        /// </summary>
        private void LoadSalesReturnData()
        {
            try
            {
                ...
                // STEP 3: Clear tables
                dsHierarchical.Tables["SalesReturnDetail"].Clear();
                dsHierarchical.Tables["SalesReturnMaster"].Clear();
                ...
                DataTable masterTable = dsHierarchical.Tables["SalesReturnMaster"];
                DataTable detailTable = dsHierarchical.Tables["SalesReturnDetail"];
                ...
                ultraGridMaster.DataMember = "SalesReturnMaster";
        ...
        /// <summary>
        /// Load master data (Sales Return Records)
        /// </summary>
        private void LoadMasterData(DateTime fromDate, DateTime toDate)
        {
            try
            {
                ...
                List<SalesReturnReportMaster> masterData = reportRepository.GetSalesReturnRecords(fromDate, toDate, branchId);
                DataTable masterTable = dsHierarchical.Tables["SalesReturnMaster"];

                foreach (var returnRecord in masterData)
                {
                    if (returnRecord.SReturnNo > 0)
                    {
                        DataRow row = masterTable.NewRow();
                        row["SReturnNo"] = returnRecord.SReturnNo;
                        row["SReturnDate"] = returnRecord.SReturnDate;
                        row["InvoiceNo"] = returnRecord.InvoiceNo ?? "";
                        row["InvoiceDate"] = returnRecord.InvoiceDate;
                        row["CustomerName"] = returnRecord.CustomerName ?? "";
        ...
        private void LoadAllDetailData(DateTime fromDate, DateTime toDate)
        {
            try
            {
                ...
                // Get all SReturnNo values from master
                var returnNumbers = dsHierarchical.Tables["SalesReturnMaster"].AsEnumerable()
                    .Select(r => r.Field<int>("SReturnNo"))
                    .ToList();

                // Load details for each return
                foreach (int sReturnNo in returnNumbers)
                {
                    SalesReturnReportData reportData = reportRepository.GetSalesReturnReportDetails(sReturnNo);
        ...
        private void UltraGridMaster_BeforeRowExpanded(object sender, CancelableRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.Band.Index == 0 && e.Row.Cells["SReturnNo"] != null)
                {
        ...
        private void UltraGridMaster_AfterRowExpanded(object sender, RowEventArgs e)
        {
            try
            {
                if (e.Row.Band.Index == 0 && e.Row.Cells["SReturnNo"] != null)
                {
                    int sReturnNo = Convert.ToInt32(e.Row.Cells["SReturnNo"].Value);
                    int detailCount = e.Row.ChildBands[0].Rows.Count;

                    System.Diagnostics.Debug.WriteLine($"Expanded Return No: {sReturnNo}, Detail Count: {detailCount}");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (dsHierarchical.Tables["SalesReturnMaster"].Rows.Count > 0)
                {
        ...
        private void PrintSalesReturnReport()
        {
            try
            {
                ...
                printDocument.DocumentName = "Sales Return Report";
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            try
            {
                ...
                // Print title
                e.Graphics.DrawString("SALES RETURN REPORT - MASTER DETAIL VIEW", titleFont, Brushes.Black, leftMargin, yPosition);
                yPosition += 30;
                ...
                string[] headers = { "Return No", "Date", "Customer", "Payment", "Sub Total", "Grand Total" };
                ...
                foreach (DataRow row in masterTable.Rows)
                {
                    ...
                    string[] values = {
                        row["SReturnNo"].ToString(),
                        Convert.ToDateTime(row["SReturnDate"]).ToString("dd/MM/yyyy"),
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                LoadSalesReturnDataWithFilters();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Load sales return data with applied filters
        /// </summary>
        private void LoadSalesReturnDataWithFilters()
        {
            try
            {
                isLoading = true;
                ...
                // Show result count
                int resultCount = dsHierarchical.Tables["SalesReturnMaster"].Rows.Count;
                if (resultCount == 0)
                {
            ...
            {
                MessageBox.Show($"Error searching sales return data: {ex.Message}\n\nStack Trace:\n{ex.StackTrace}", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
        ...
        private void RebuildDataSetWithFilters(DateTime fromDate, DateTime toDate)
        {
            try
            {
                ...
                DataRelation relation = new DataRelation(
                    "MasterDetail",
                    masterTable.Columns["SReturnNo"],
                    dsHierarchical.Tables["SalesReturnDetail"].Columns["SReturnNo"],
                    true
                );
                dsHierarchical.Relations.Add(relation);
        ...
        /// </summary>
        private List<SalesReturnReportMaster> ApplyMasterFilters(List<SalesReturnReportMaster> masterData)
        {
            ...
            if (ultraNumericEditorReturnNo.Value != null && Convert.ToInt32(ultraNumericEditorReturnNo.Value) > 0)
            {
                ...
                filtered = filtered.Where(x => x.SReturnNo == returnNo);
            }
        ...
        private void LoadDetailsForFilteredReturns()
        {
            try
            {
                ...
                // Get all SReturnNo values from filtered master
        ...
        private void ExportToExcel()
        {
            try
            {
                ...
                saveDialog.FileName = $"SalesReturnReport_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        {
            DataTable exportTable = new DataTable("SalesReturnReport");

            // Add columns
            exportTable.Columns.Add("SReturnNo", typeof(int));
            exportTable.Columns.Add("SReturnDate", typeof(DateTime));
            ...
            exportTable.Columns.Add("SalesPrice", typeof(decimal));
            ...
            foreach (DataRow masterRow in dsHierarchical.Tables["SalesReturnMaster"].Rows)
            {
                DataRow[] detailRows = dsHierarchical.Tables["SalesReturnDetail"].Select($"SReturnNo = {masterRow["SReturnNo"]}");
                ...
                if (detailRows.Length > 0)
                {
                    foreach (DataRow detailRow in detailRows)
                    {
                        ...
                        exportRow["SReturnNo"] = masterRow["SReturnNo"];
                        exportRow["SReturnDate"] = masterRow["SReturnDate"];
                        ...
                        exportRow["SalesPrice"] = detailRow["SalesPrice"];
