...
namespace PosBranch_Win.Accounts
{
    public partial class FrmJournal : Form
    {
        ...
        private void StyleGrid()
        {
            ...
            dgvJournal.DisplayLayout.BorderStyle = UIElementBorderStyle.None;
            dgvJournal.DisplayLayout.CaptionVisible = DefaultableBoolean.False;
            dgvJournal.DisplayLayout.GroupByBox.Hidden = true;
            dgvJournal.DisplayLayout.AutoFitStyle = AutoFitStyle.ResizeAllColumns;
            dgvJournal.DisplayLayout.Appearance.BackColor = Color.White;
            dgvJournal.DisplayLayout.Override.HeaderAppearance.BackColor = Color.FromArgb(18, 65, 89);
            dgvJournal.DisplayLayout.Override.HeaderAppearance.BackColor2 = Color.FromArgb(18, 65, 89);
            dgvJournal.DisplayLayout.Override.HeaderAppearance.BackGradientStyle = GradientStyle.None;
            dgvJournal.DisplayLayout.Override.HeaderAppearance.ForeColor = Color.White;
            dgvJournal.DisplayLayout.Override.HeaderAppearance.FontData.Bold = DefaultableBoolean.True;
            dgvJournal.DisplayLayout.Override.HeaderAppearance.TextHAlign = HAlign.Center;
            dgvJournal.DisplayLayout.Override.HeaderStyle = HeaderStyle.WindowsXPCommand;
            dgvJournal.DisplayLayout.Override.HeaderClickAction = HeaderClickAction.Select;
            dgvJournal.DisplayLayout.Override.RowAppearance.BackColor = Color.White;
            dgvJournal.DisplayLayout.Override.RowAlternateAppearance.BackColor = Color.FromArgb(248, 251, 252);
            dgvJournal.DisplayLayout.Override.ActiveRowAppearance.BackColor = Color.FromArgb(219, 234, 254);
            dgvJournal.DisplayLayout.Override.ActiveCellAppearance.BackColor = Color.FromArgb(239, 246, 255);
            dgvJournal.DisplayLayout.Override.CellAppearance.ForeColor = Color.FromArgb(31, 42, 55);
            dgvJournal.DisplayLayout.Override.CellPadding = 6;
            dgvJournal.DisplayLayout.Override.RowSelectorWidth = 34;
            dgvJournal.DisplayLayout.Override.RowSelectorHeaderStyle = RowSelectorHeaderStyle.ColumnChooserButton;
            dgvJournal.DisplayLayout.Override.CellClickAction = CellClickAction.EditAndSelectText;
            dgvJournal.DisplayLayout.Override.AllowAddNew = AllowAddNew.TemplateOnBottom;
            dgvJournal.DisplayLayout.Override.AllowDelete = DefaultableBoolean.True;
            dgvJournal.DisplayLayout.Override.SelectTypeRow = SelectType.Single;
            dgvJournal.DisplayLayout.Override.RowSelectors = DefaultableBoolean.True;
        ...
        {
            if (ledgerTable == null || dgvJournal.DisplayLayout.Bands.Count == 0)
            {
                return;
            }
            ...
            UltraGridBand band = dgvJournal.DisplayLayout.Bands[0];
        ...
        private JournalVoucher BuildJournalFromGrid()
        {
        ...
        private bool ValidateJournalForSave(JournalVoucher journal)
        {
            ClearRowErrors();

            if (journal.Lines.Count < 2)
            {
                MessageBox.Show("Please enter at least two journal lines.", "Validation",
            ...
            if (!valid)
            {
                MessageBox.Show("Please fix highlighted journal lines.", "Validation",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            ...
            {
                MessageBox.Show("Total Debit must equal Total Credit before saving.", "Journal Not Balanced",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
        ...
        private void LoadJournalToForm(JournalVoucher journal)
        {
            isBinding = true;
        ...
        private void SaveJournal(bool requireExisting)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Load an existing journal before updating.", "Information",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                JournalVoucher journal = BuildJournalFromGrid();
                if (!ValidateJournalForSave(journal))
                {
                    UpdateTotals();
                    return;
                }
        ...
        private void LoadJournal()
        {
            try
            {
                ...
                JournalVoucher journal = journalRepository.GetJournalVoucher(txtVoucherNo.Text.Trim());
                if (journal == null)
                {
                    MessageBox.Show("Journal voucher not found.", "Information",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                LoadJournalToForm(journal);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error loading journal voucher: {ex.Message}", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void DeleteJournal()
        {
            ...
            {
                MessageBox.Show("Load a journal voucher before deleting.", "Information",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            DialogResult result = MessageBox.Show("Delete this journal voucher?", "Confirm Delete",
            ...
            try
            {
                journalRepository.Delete(currentVoucherId);
                ClearForm();
        ...
        private void dgvJournal_AfterCellUpdate(object sender, CellEventArgs e)
        {
